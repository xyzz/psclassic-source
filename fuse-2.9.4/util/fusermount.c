/*
  ...
  Changes made 2014 by Anton Feichtmeir <Tong.Duurai@gmail.com>:
  
  GCC compiler errors debugged and the newly created header fuse-2.9.4/lib/mount.h 
  is used to provide the definitions of mount_util functions. The mount_util.c
  source code from the /util directory was removed and its contents integrated into 
  the fuse-2.9.4/lib/mount.c source file. This helps us to avoid conflicting function definitions.
...
{
	if (getuid() != 0) {
		oldfsuid = setfsuid(syscall(__NR_getuid));
		oldfsgid = setfsgid(syscall(__NR_getgid));
	}
}
...
/*A duplicate of a static int mtab_needs_update redefined here for use by fusermount functions. */

static int __mtab_needs_update(const char *mnt)
{
	int res;
	struct stat stbuf;
...
static inline int mnt_add_mount(const char *progname, const char *fsname,
...
int fuse_add_mount(const char *progname, const char *fsname,
		       const char *mnt, const char *type, const char *opts)
{
	if (!__mtab_needs_update(mnt))
		return 0;

	return mnt_add_mount(progname, fsname, mnt, type, opts);
}
...
int fuse_remove_mount(const char *progname, const char *mnt)
{
	if (!__mtab_needs_update(mnt))
		return 0;
...
int mnt_check_empty(const char *progname, const char *mnt,
...
inline char *mnt_resolve_path(const char *progname, const char *orig)
{
	char buf[PATH_MAX];
	char *copy;
	char *dst;
...
inline int fuse_mnt_check_fuseblk(void)
{
	char buf[256];
...
static int add_mount(const char *source, const char *mnt, const char *type,
		     const char *opts)
{
	return fuse_add_mount(progname, source, mnt, type, opts);
}
...
static int check_is_mount_child(void *p)
{
	...
	res = syscall(__NR_mount,".", "/", "", MS_BIND | MS_REC, NULL);
	if (res == -1) {
...
{
	int res = syscall(__NR_umount2,"", UMOUNT_UNUSED);
	if (res != -1 || errno != EINVAL)
		return 0;
        res = syscall(__NR_umount2,"", UMOUNT_NOFOLLOW);
...
static int unmount_fuse_locked(const char *mnt, int quiet, int lazy)
{
	...
	if (umount_nofollow_support()) {
		...
		if (res == -1)
                        perror("check_is_mount");
			goto out;
	}

	res = syscall(__NR_umount2,last, umount_flags);
	if (res == -1 && !quiet) {
		    ...
		    char **mnt_optsp, off_t rootsize)
{
	...
	*d = '\0';
	...
	if (check_empty &&
	    mnt_check_empty(progname, mnt, rootmode, rootsize) == -1)
		goto err;
...
int main(int argc, char *argv[])
{
	...
	drop_privs();
	mnt = mnt_resolve_path(progname, origmnt);
	if (mnt != NULL) {
		res = chdir("/");
		if (res == -1) {
			fprintf(stderr, "%s: failed to chdir to '/'\n", progname);
...
do_unmount:
	if (geteuid() == 0)
		res = unmount_fuse(mnt, quiet, lazy);
                ...
                int flags = lazy ? UMOUNT_DETACH : 0;
		res = syscall(__NR_umount2,mnt,flags);
		if (res == -1 && !quiet)
			fprintf(stderr,
